...
class Main extends React.Component {
  static propTypes = {
    additionalStargazers: React.PropTypes.any,
    transmit: React.PropTypes.any,
    stargazers: React.PropTypes.any
  };
  /**
  ...
  render() {
     ...
     */
    let { stargazers } = this.props;
    const { additionalStargazers } = this.props;
    ...
    return (
      <InlineCss stylesheet={Main.css(avatarSize)} namespace="Main">
        ...
        <div><Link to="/hello">hello-----------------------------</Link></div>
          ...
          <br/>React Isomorphic Starterkit. Let&apos;s get you started! wtf
        </h1>
...
export default Transmit.createContainer(Main, {
  ...
  fragments: {
    ...
    additionalStargazers: ({ nextPage, additionalStargazers }) =>
      () => fetchStargazers(nextPage).then(newStargazers => {
        const modifiedNewStargazers = newStargazers.map(({ id, login }) => ({ id, login }));
        return additionalStargazers.concat(modifiedNewStargazers);
      })
  }
});
